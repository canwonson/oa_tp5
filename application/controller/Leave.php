...
class Leave extends Flow
{
	...
	{
		return ['upload', 'date_time', 'sweetalert'];
	}
	...
	public function index()
	{
		...
		$datas = model('leave')->field('user_id, count(*) count, sum(leave_day) as leave_day')->where(['status'=>1])->where($where)->group('user_id')->select();
	...
	protected function saveData($data, $insert = 1)
	{
		$save_data = [
			'user_id'    => get_user_id(),
		...
		$insert == 1 && $save_data['flow_id'] = $data['flow_id'];
		return $save_data;
	}
	...
	public function getOvertime()
	{
		$user_id   = get_user_id();
		$available = model('Overtime')->where(['user_id'=>$user_id, 'status'=>1])->sum('overtime');
		$used      = model('leave')->where(['user_id'=>$user_id, 'status'=>1, 'leave_type'=>4, 'status' => ['notin', [2, 6]]])->sum('leave_day');
		$data      = [
			'available' => $available,
			'used'      => $used,
		];
	...
	private function annualLeave($data)
	{
		...
		if ($diff_day <= 0) {
			//最小年度足够扣除年假
			$used_day = $min_info['used_day'] + $data['leave_day'];
			$Annual->where(['user_id' => get_user_id(), 'year' => $min_info['year'], 'status' => 1])->update(['used_day'=> $used_day]);
			//插入日志
			$data_log = [
	...
	{
		$this->delAnnualLog($flow_id);
	}

	private function delAnnualLog($flow_id){
		$this->annualBack($flow_id);
		$AnnualLog = model('Annual_log');
		$AnnualLog->where(['flow_id'=>$flow_id,'status'=>0,'is_del'=>0])->update(['is_del'=>1]);
	}

	//年假回退
	public function annualBack($flow_id)
	{
		$Annual    = model('Annual');
		$AnnualLog = model('Annual_log');
		$user_id   = model('Flow')->where(['id'=>$flow_id])->value('user_id');
		$log_list  = $AnnualLog->where(['flow_id'=>$flow_id,'status'=>['in', [0, 2, 6]],'is_del'=>0])->column('leave_day', 'year');
		//年假表回退
		if ($log_list) {
	...
	public function extChange($data, $status = 1)
	{
		switch ($data['leave_type']) {
			case '3':
				$this->changeAnnualLeave($data['flow_id'], $status);
				break;
