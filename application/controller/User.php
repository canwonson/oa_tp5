...
class User extends Base
{
	...
	{
		$plugin = ['titatoggle', 'sweetalert', 'date'];
		set_url('/user/index');
		$param = input('param.');
		$where = [];
		$where['is_del'] = 0;
		...
		(isset($param['entry_start_time']) && isset($param['entry_end_time']) && !empty($param['entry_start_time']) && !empty($param['entry_end_time'])) && $where['entry_time'] = ['between', [strtotime($param['entry_start_time']), strtotime($param['entry_end_time'])+ 86400-1]];
        // (isset($param['entry_start_time']) && !isset($param['entry_end_time']) && !empty($param['entry_start_time'])) && $where['entry_time'] = ['>=', strtotime($param['entry_start_time'])];
        // (!isset($param['entry_start_time']) && isset($param['entry_end_time']) && !empty($param['entry_end_time'])) && $where['entry_time'] = ['<=', strtotime($param['entry_end_time'])+ 86400-1];
		(isset($param['leave_start_time']) && isset($param['leave_end_time']) && !empty($param['leave_start_time']) && !empty($param['leave_end_time'])) && $where['leave_time'] = ['between', [strtotime($param['leave_start_time']), strtotime($param['leave_end_time'])+ 86400-1]];
        // (isset($param['leave_start_time']) && !isset($param['leave_end_time']) && !empty($param['leave_start_time'])) && $where['leave_time'] = ['>=', strtotime($param['leave_start_time'])];
        // (!isset($param['leave_start_time']) && isset($param['leave_end_time']) && !empty($param['leave_end_time'])) && $where['leave_time'] = ['<=', strtotime($param['leave_end_time'])+ 86400-1];
		$datas = model('User')->where($where)->select();
		$conf_list = $this->getConfList();
		$conf_list['user_type'] = self::$user_type;
		return $this->fetch('index',['datas'=>$datas, 'conf_list'=>$conf_list, 'param'=>$param, 'plugin'=>$plugin, 'types' => self::$user_type]);
	}
	...
	public function getAccount()
	{
		...
		return implode($Pinyin->name($name));
	}
